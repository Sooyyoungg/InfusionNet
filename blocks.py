...
# 3. AdaOctConv
###############################################################################################
def model_save(save_type, ckpt_dir, model, optim_E, optim_S, optim_G, epoch, itr=None, ssim=None):
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)

    if itr is not None:
        torch.save({'netE': model.module.netE.state_dict(),
                    'netS': model.module.netS.state_dict(),
    ...
    if save_type == 'best':
        torch.save({'netE': model.module.netE.state_dict(),
                    ...
                    'optim_S': optim_S.state_dict(),
                    'optim_G': optim_G.state_dict()},
                   '%s/model_best_iter_%d_epoch%d_ssim_%f.pth' % (ckpt_dir, itr+1, epoch+1, ssim))

def model_load(save_type, checkpoint, ckpt_dir, model, optim_E, optim_S, optim_G):
    if not os.path.exists(ckpt_dir):
        epoch = -1
    ...
    model.module.netE.load_state_dict(dict_model['netE'])
    model.module.netS.load_state_dict(dict_model['netS'])
    model.module.netG.load_state_dict(dict_model['netG'])
...
###############################################################################################
## Encoder: Octave Convolution (also used in Decoder)
###############################################################################################
class OctConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, 
                 padding=0, groups=1, pad_type='reflect', alpha_in=0.5, alpha_out=0.5, type='normal', freq_ratio = None):
    ...
    def forward(self, x):
        if self.type == 'first':
            ...
            output = out_h * self.freq_ratio[1] + out_l * self.freq_ratio[0]
...
class AdaOctConv(nn.Module):
    def __init__(self, in_channels, out_channels, group_div, style_channels, kernel_size,
        ...
        style_channels_h = style_channels[0]
        style_channels_l = style_channels[1]
    ...
    def forward(self, content, style):
        ...
        kernel_pred = [[h_w_spatial, h_w_pointwise, h_bias], [l_w_spatial, l_w_pointwise, l_bias]]
